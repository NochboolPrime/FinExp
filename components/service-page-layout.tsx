...
export function ServicePageLayout({ title, children }: ServicePageLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] flex flex-col">
      ...
      {/* Footer */}
      <footer className="py-4 sm:py-8 bg-gray-200/50 backdrop-blur-xl border-t border-gray-200/50">
        <div className="max-w-6xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
              ...
              <ul className="space-y-0 sm:space-y-1">
                {navLinks.map((link) => (
                  <li key={link.name}>
                    <Link
                      href={link.href}
                      className="footer-link text-xs text-gray-400 hover:text-[#1a2a3a] transition-colors inline-block"
                    >
                      {link.name}
                    </Link>
                  </li>
